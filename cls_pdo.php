...
// class cls_pdo extends cls_mysql{
class cls_pdo {
    var $link_id    = NULL;
    ...
    {
        $this->cls_pdo($dbhost, $dbuser, $dbpw, $dbname, $charset, $pconnect, $quiet);
    
    }
    
    function cls_pdo($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'gbk', $pconnect = 0, $quiet = 0)
    {
    ...
    function connect($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0)
    {
        ...
        {
                try {
                    $this->link_id =new PDO('mysql:dbname='.$dbname.';host='.$dbhost, $dbuser, $dbpw,array(PDO::ATTR_PERSISTENT=> true));
                }catch (PDOException $e){
                    $this->ErrorMsg("Can't Connect MySQL Server($dbhost)!".$e->getMessage());
                    return false;
                }
        }
        else
        {
            ...
            {
                try {
                    $this->link_id =new PDO('mysql:dbname='.$dbname.';host='.$dbhost, $dbuser, $dbpw);
                }catch (PDOException $e){
                    $this->ErrorMsg("Can't Connect MySQL Server($dbhost)!".$e->getMessage());
                    return false;
            ...
            {
                //php5以下不支持pdo
                $this->ErrorMsg("Can't Connect MySQL Server($dbhost)!,php version<5");
...
//                 mt_srand((double)microtime() * 1000000); // 对 PHP 4.2 以下的版本进行随机数函数的初始化工作
        ...
        $this->version = $this->link_id->getAttribute(PDO::ATTR_SERVER_VERSION);
        ...
        if ($this->version > '4.1')
        {
            if ($charset != 'latin1')
            {
                $this->link_id->query("SET character_set_connection=$charset, character_set_results=$charset, character_set_client=binary");
            }
...
//             mysql_ping($this->link_id);
           /*暂未确定修改，  $this->settings  已被该方法开头置为空*/
//             $status = $this->link_id->getAttribute(PDO::ATTR_SERVER_INFO);         
...
//             }
        ...
        if (!($query = $this->link_id->query($sql)) && $type != 'SILENT')
        {
            ...
            $this->error_message[]['error'] = $this->link_id->errorInfo();
            $this->error_message[]['errno'] = $this->link_id->errorCode();
            $this->ErrorMsg(json_encode($this->error_message),$sql);
            return false;
        }
    ...
    /**
     * 待修改   trunk一两处使用  pdo需要query对象调用影响的行
     * @see cls_mysql::affected_rows()
     */
    ...
    function error()
    {
        return $this->link_id->errorInfo();
    }
    
    function errno()
    {
        return $this->link_id->errorCode();
    }

    //trunk 版本中未使用过
    function result($query, $row)
    {
//         return @mysql_result($query, $row);//mysql原始，新方法暂未找到
    }
    
    //trunk使用一次  goods.php line 755
    function num_rows($query)
    {
//            return $db->getAttribute(PDO::MYSQL_ATTR_FOUND_ROWS);
...
//         return mysql_num_rows($query);
    ...
    {
//         return $query->columnCount();//PDO新方法
//         return mysql_num_fields($query);//mysql 原始
    }
    
    //trunk 版本中未使用过
    function free_result($query)
    {
//         return $query->closeCursor();//pdo新方法
//         return mysql_free_result($query);//mysql 原始
    }
    ...
    /**
     * 待修改  admin integrate.php line 454 使用一次
     * @return boolean
     */
    function ping()
    {
...
//             }
    ...
    function table_lastupdate($tables)
    {
        ...
        $lastupdatetime = strtotime($lastupdatetime) - $this->timezone + $this->timeline;
           //应该修改统一返回时间格式， 原版返回时间戳或'2037-12-31 23:59:59';
